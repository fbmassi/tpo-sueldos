...
# la fuente de matplotlib no tiene emojis: la leyenda usa nombres sin emoji
planos = {c: re.sub(r"[^\w\s+#/&.-]", "", n).strip() for c, n in nombres.items()}
for c in range(k):
    m = df["cluster"].values[samp] == c
    ax.scatter(coords[samp][m, 0], coords[samp][m, 1], c=PALETA[c % len(PALETA)],
               s=18, alpha=0.6, edgecolors="none", label=planos[c])
